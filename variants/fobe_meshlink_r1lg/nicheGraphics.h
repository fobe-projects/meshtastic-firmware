...
void setupNicheGraphics()
{
    ...
    inkhud->persistence->settings.rotation = 2;           // 270 degrees clockwise
    ...
    buttons->setWiring(MAIN_BUTTON, PIN_BUTTON1);
    buttons->setTiming(MAIN_BUTTON, 50, 500);
    buttons->setHandlerShortPress(MAIN_BUTTON, []() { InkHUD::InkHUD::getInstance()->longpress(); });
    buttons->setHandlerLongPress(MAIN_BUTTON, []() { InkHUD::InkHUD::getInstance()->nextTile(); });

    buttons->setWiring(AUX_BUTTON, PIN_BUTTON2);
    buttons->setTiming(AUX_BUTTON, 50, 500);
    buttons->setHandlerShortPress(AUX_BUTTON, []() { InkHUD::InkHUD::getInstance()->shortpress(); });
    buttons->setHandlerLongPress(AUX_BUTTON, []() { InkHUD::InkHUD::getInstance()->updateLayout(); });
    buttons->start();
}
